...
class Gameboard {
  ...
  rotateShip(shipId, point) {
    ...
    let [headX, headY] =
      point > location[0] ? location[location.length - 1] : location[0];
  ...
  validPlace(coordinates) {
    for (let [x, y] of coordinates) {
      if (
        !h.validCoordinates(x, y) ||
        !h.adjacentEmpty(x, y, this.board) ||
        this.board[x][y] !== null
      ) {
        return false;
      }
  ...
  placeRandom(ship) {
    ...
    coordinates.forEach((shipCell) => {
      ...
      h.adjacent(shipCell[0], shipCell[1]).forEach((cell) =>
        this.occupiedCells[cell] ? "" : (this.occupiedCells[cell] = true),
      );
    });
